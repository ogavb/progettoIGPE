...
public class SchermataNuovaPartitaMultiPlayer {
   ...
   public SchermataNuovaPartitaMultiPlayer(boolean isClient, Client client,
   ...
   private void drawSelectPlayer(int nGioc) {
      ...
      primaryStage.setOnCloseRequest(event -> {
         ...
         if (isClient && rmc.getFinestraMultiPlayer() != null) {
            ...
            client.addRequest(
                  "7##aggiorna rimozione giocatore finestra dei match");
            ...
            primaryStage = ((Stage) ((Button) event.getSource()).getScene()
                  .getWindow());
            ...
            Parent root = rmc.getFinestraMultiPlayer().show(primaryStage,
                  rmc.getMatchs());
      ...
      conferma.setOnAction(new EventHandler<ActionEvent>() {
         ...
         public void handle(ActionEvent event) {
            ...
            if (errorFlag == true) {
               ...
               String nomeConfigurazione = ((String) configurazioni
               ...
               for (Giocatore g : gm.getGiocatori()) {
                  System.err
                        .println("Ordine giocatore " + g.getOrdineDiPartenza());
               }

               for (int i = 0; i < giocatori.length; i++) {
                  if (i == giocatori.length - 1) {
                     giocatori[i] = gm.getGiocatore(i);
                     nomiGiocatori.append(giocatori[i].getNome() + "("
                           + giocatori[i].getColor() + "("
                           + giocatori[i].getOrdineDiPartenza());
                  }
                  else {
                     giocatori[i] = gm.getGiocatore(i);
                     nomiGiocatori.append(giocatori[i].getNome() + "("
               ...
               client.addRequest("5##" + nomeConfigurazione + "/"
                     + nomiGiocatori.toString() + "/" + numeroGiocatori);
   ...
   public void initGameManager(Giocatore[] nomiGiocatori, int numeroGiocatori,
