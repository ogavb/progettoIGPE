...
public class GameManagerNetwork extends GameManagerAstratta {
   ...
   public void turnoSuccessivo(int lancioCorrente) {
      ...
      // OutputMediator.println( corrente.getPos().getX() + " " +
      // corrente.getPos().getY() );
      controllaCasella(corrente);
      if (!isYourRound)
         client.addRequest(
               "13##" + gestore.getNextPlayer(corrente.getNome()) + "##10");
   }
   ...
   public void finePartita() {
      ...
      OutputMediator.println("Il vincitore è: " + vincitore.getNome() + " con "
            + vincitore.getCrediti() + " crediti e con "
   ...
   protected void updatePosizioneGiocatoreIndietro(Giocatore g) {
      ...
      if (g.getRisultatoDado() > g.getPos().getY()) {
         ...
         g.getPos().setY(tdg.getTavola()[0].length
               - (g.getRisultatoDado() - g.getPos().getY()));
      }
      else
         g.getPos().setY(g.getPos().getY() - g.getRisultatoDado());
