...
public class PannelloTavola extends GridPane implements PaneSwitcher {
   ...
   private ImageView iv4 = new ImageView(
         new Image("file:giocatori/purple.gif"));
   private ImageView iv5 = new ImageView(new Image("file:giocatori/red.gif"));
   private ImageView iv6 = new ImageView(
         new Image("file:giocatori/yellow.gif"));
   ...
   public MyPane makePanel() {
      ...
      MyPane p = new MyPane(tdg.getCasella(riga, colonna),
            mappa.get(nomeCasella));
   ...
   /*
    * Sposta l'image view del giocatore g nella nuova posizione mediante
    * animazioni
    */
   public void spostaGiocatore(Giocatore g) {
      /*
       * Converto la posizione reale del giocatore nella corrispondente
       * posizione della tavola da gioco
       */
       ...
       */
      MyPane mp = (MyPane) getNodeByRowColumnIndex(pnuova.getY(),
            pnuova.getX());
      ...
      /*
       * se l'immagine n si trova sul lato TOP della tavola e la casella dove
       * l'immagine andrà a posizionarsi si trova sul lato RIGHT, allora si crea
       * un PATH che va dalla posizione corrente dell'immagine sul TOP alla
       * posizione dell'ultima casella TOP e da qui alla posizione della casella
       * dove andrà a posizionarsi l'immagine sul lato RIGHT.
       *
      ...
      if (!(n.getTranslateX() + 11 == 11 && n.getTranslateY() + 12.5 == 12.5)
            && ((n.getTranslateX() + 11 >= 11
                  && n.getTranslateX() + 11 <= (mp.getWidth() * 10) + 11)
                  && (n.getTranslateY() + 12.5 == 12.5))
            && (mp.getLayoutY() >= mp.getHeight())) {

         path.getElements().add(
               new MoveTo(n.getTranslateX() + 11, n.getTranslateY() + 12.5));
         path.getElements().add(new LineTo((mp.getWidth() * 10) + 11, 12.5));
         path.getElements().add(
               new LineTo((mp.getWidth() * 10) + 11, mp.getLayoutY() + 12.5));

      }

      /*
       *
       * se l'immagine n si trova sul lato RIGHT della tavola e la casella dove
       * l'immagine andrà a posizionarsi si trova sul lato BOTTOM, allora si
       * crea un PATH che va dalla posizione corrente dell'immagine sul RIGHT
       * alla posizione dell'ultima casella RIGHT e da qui alla posizione della
       * casella dove andrà a posizionarsi l'immagine sul lato BOTTOM.
       *
       * Il percorso è del tipo:
       *
       * | | | \ /
      ...
      else if (!(n.getTranslateX() + 11 == 11
            && n.getTranslateY() + 12.5 == 12.5)
            && (n.getTranslateX() + 11 == (mp.getWidth() * 10) + 12)
            && ((n.getTranslateY() + 12.5 >= 12.5
                  && n.getTranslateY() + 12.5 < (mp.getHeight() * 10) + 12.5))
            && (mp.getLayoutX() < (mp.getWidth() * 10))) {

         path.getElements().add(
               new MoveTo(n.getTranslateX() + 11, n.getTranslateY() + 12.5));
         path.getElements().add(new LineTo((mp.getWidth() * 10) + 11,
               (mp.getHeight() * 10) + 12.5));
         path.getElements()
               .add(new LineTo(mp.getLayoutX() + 11, mp.getLayoutY() + 12.5));
       ...
       *
       *
       * se l'immagine n si trova sul lato BOTTOM della tavola e la casella dove
       * l'immagine andrà a posizionarsi si trova sul lato LEFT, allora si crea
       * un PATH che va dalla posizione corrente dell'immagine sul BOTTOM alla
       * posizione dell'ultima casella BOTTOM e da qui alla posizione della
       * casella dove andrà a posizionarsi l'immagine sul lato LEFT.
       *
       * Il percorso è del tipo:
       *
       *
      ...
      /*
       * se l'immagine n si trova sul lato LEFT della tavola e la casella dove
       * l'immagine andrà a posizionarsi si trova sul lato TOP, allora si crea
       * un PATH che va dalla posizione corrente dell'immagine sul LEFT alla
       * posizione dell'ultima casella LEFT e da qui alla posizione della
       * casella dove andrà a posizionarsi l'immagine sul lato TOP.
       *
       * Il percorso è del tipo:
       *
       *
       ...
       */
            ...
            && n.getTranslateY() + 12.5 == 12.5)
            && ((n.getTranslateX() + 11 == 12)
                  && (n.getTranslateY() + 12.5 > 12.5 && n.getTranslateY()
                        + 12.5 <= (mp.getHeight() * 10) + 12.5))
            && (mp.getLayoutX() > 12)) {
       ...
       *
       * 1) orizzontale da sinistra verso destra => TOP 2) verticale dall'alto
       * verso il basso => RIGHT 3) orizzontale da destra verso sinistra =>
       * BOTTOM 4) verticale dal basso verso l'ato => LEFT
       */
      ...
      pathTransition[this.numPathTransition] = new PathTransition(
            Duration.millis(1000), path, n);
      ...
      pathTransition[this.numPathTransition].setOnFinished(event -> {
         ...
         String nomeCasella = tdg
               .getCasella(g.getPos().getX(), g.getPos().getY()).toString();

         Casella casellaCorrente = null;
         if (nomeCasella.equals("esame"))
            casellaCorrente = tdg.getCasella(g.getPos().getX(),
                  g.getPos().getY());
   ...
   /*
    * Il metodo setCoordinatesSingleImageView prende in input un MyPane
    * corrispondente alla casella dove risiede il giocatore e l'image view del
    * giocatore e imposta le coordinate dell'image view.
    */
   ...
   private void setCoordinatesSingleImageView(MyPane p, ImageView i) {
       ...
       */
      if (((p.getLayoutY() + 12.5 == 12.5) && ((p.getLayoutX() + 11 >= 11
            && p.getLayoutX() + 11 <= (p.getLayoutX() * 10) + 11)))) {
         ...
         // dalla casella (0,0) trasli l'image view nella casella dove si
         // trova il giocatore
      ...
      else if (((p.getLayoutX() + 11 == (p.getLayoutX() * 10) + 11)
            && ((p.getLayoutY() + 12.5 >= p.getHeight() + 12.5
                  && p.getLayoutY() + 12.5 <= (p.getHeight() * 10) + 12.5)))) {
      ...
      else if (((p.getLayoutY() + 12.5 == (p.getHeight() * 10) + 12.5)
            && ((p.getLayoutX() + 11 >= 11
                  && p.getLayoutX() + 11 <= (p.getWidth() * 10) + 11)))) {
   ...
   /*
    * il metodo setCoordinatesImageView prende in input un indice di colonna e
    * un indice di riga. Verifica se nella casella puntata da columnIndex e
    * rowIndex si trovano uno o più giocatori; in caso affermativo vengono
    * impostate le coordinate sulla tavola di gioco dell'image view del
    * giocatore corrispondente invocando il metodo setCoordinatesSingleImageView
    */
   ...
   private Posizione convertiPosizione(Posizione p) {
      if (p.getX() == 0 && p.getY() == 0)
         return new Posizione(0, 0);
      else if (p.getX() == 0 && p.getY() == 1)
         return new Posizione(1, 0);
      else if (p.getX() == 0 && p.getY() == 2)
         return new Posizione(2, 0);
      else if (p.getX() == 0 && p.getY() == 3)
         return new Posizione(3, 0);
      else if (p.getX() == 0 && p.getY() == 4)
         return new Posizione(4, 0);
      else if (p.getX() == 0 && p.getY() == 5)
         return new Posizione(5, 0);
      else if (p.getX() == 0 && p.getY() == 6)
         return new Posizione(6, 0);
      else if (p.getX() == 0 && p.getY() == 7)
         return new Posizione(7, 0);
      else if (p.getX() == 0 && p.getY() == 8)
         return new Posizione(8, 0);
      else if (p.getX() == 0 && p.getY() == 9)
         return new Posizione(9, 0);

      else if (p.getX() == 1 && p.getY() == 0)
         return new Posizione(NUM_COLS - 1, 0);
      else if (p.getX() == 1 && p.getY() == 1)
         return new Posizione(NUM_COLS - 1, 1);
      else if (p.getX() == 1 && p.getY() == 2)
         return new Posizione(NUM_COLS - 1, 2);
      else if (p.getX() == 1 && p.getY() == 3)
         return new Posizione(NUM_COLS - 1, 3);
      else if (p.getX() == 1 && p.getY() == 4)
         return new Posizione(NUM_COLS - 1, 4);
      else if (p.getX() == 1 && p.getY() == 5)
         return new Posizione(NUM_COLS - 1, 5);
      else if (p.getX() == 1 && p.getY() == 6)
         return new Posizione(NUM_COLS - 1, 6);
      else if (p.getX() == 1 && p.getY() == 7)
         return new Posizione(NUM_COLS - 1, 7);
      else if (p.getX() == 1 && p.getY() == 8)
         return new Posizione(NUM_COLS - 1, 8);
      else if (p.getX() == 1 && p.getY() == 9)
         return new Posizione(NUM_COLS - 1, 9);

      else if (p.getX() == 2 && p.getY() == 0)
         return new Posizione(10, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 1)
         return new Posizione(9, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 2)
         return new Posizione(8, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 3)
         return new Posizione(7, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 4)
         return new Posizione(6, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 5)
         return new Posizione(5, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 6)
         return new Posizione(4, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 7)
         return new Posizione(3, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 8)
         return new Posizione(2, NUM_COLS - 1);
      else if (p.getX() == 2 && p.getY() == 9)
         return new Posizione(1, NUM_COLS - 1);

      else if (p.getX() == 3 && p.getY() == 0)
         return new Posizione(0, 10);
      else if (p.getX() == 3 && p.getY() == 1)
         return new Posizione(0, 9);
      else if (p.getX() == 3 && p.getY() == 2)
         return new Posizione(0, 8);
      else if (p.getX() == 3 && p.getY() == 3)
         return new Posizione(0, 7);
      else if (p.getX() == 3 && p.getY() == 4)
         return new Posizione(0, 6);
      else if (p.getX() == 3 && p.getY() == 5)
         return new Posizione(0, 5);
      else if (p.getX() == 3 && p.getY() == 6)
         return new Posizione(0, 4);
      else if (p.getX() == 3 && p.getY() == 7)
         return new Posizione(0, 3);
      else if (p.getX() == 3 && p.getY() == 8)
         return new Posizione(0, 2);
