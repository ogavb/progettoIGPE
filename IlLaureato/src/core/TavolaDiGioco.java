...
public class TavolaDiGioco {
   ...
   public TavolaDiGioco(GameManagerAstratta gm, String nomeConfigurazione)
         throws SQLException {

      this.setNomeConfigurazione(gm, nomeConfigurazione);
   }

   public void setNomeConfigurazione(GameManagerAstratta gm,
      ...
      for (int x = 0, i = 0, j = 0; x < configurazione.length(); x++, j++) {
         if (st.hasMoreTokens()) {
            ...
            switch (parola) {
               ...
               case "Biblioteca":
                  this.tavola[i][j] = new CasellaBiblioteca(
                        new AzioneRilanciaDado(gm), "biblioteca");
                  break;

               case "BookCafe":
                  this.tavola[i][j] = new CasellaBookCafe(
                        new AzioneRilanciaDado(gm), "bookCafe");
                  break;

               case "CentroResidenziale":
                  this.tavola[i][j] = new CasellaCentroResidenziale(
                        new AzioneFermaTurno(gm, 3), "centroResidenziale");
                  break;

               case "Cus":
                  this.tavola[i][j] = new CasellaCus(new AzioneNulla(gm),
                        "cus");
                  break;

               case "Esame":
                  this.tavola[i][j] = new CasellaEsame(new AzioneDomanda(gm),
                        "esame");
                  break;

               case "Mensa":
                  this.tavola[i][j] = new CasellaMensa(
                        new AzioneFermaTurno(gm, 1), "mensa");
                  break;

               case "Ricevimento":
                  this.tavola[i][j] = new CasellaRicevimento(
                        new AzioneFermaTurno(gm, 2), "ricevimento");
                  break;

               case "Semplice":
                  this.tavola[i][j] = new CasellaSemplice(new AzioneNulla(gm),
                        "semplice");
                  break;
